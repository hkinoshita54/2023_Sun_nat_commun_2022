...
# load data of NK cells----
cts <- ReadMtx(mtx = "downloaded/NK_cells/matrix.mtx.gz", features = "downloaded/NK_cells/features.tsv.gz", cells = "downloaded/NK_cells/barcodes.tsv.gz")
meta <- read.delim("downloaded/NK_cells/meta_data.tsv", stringsAsFactors = F, header = T)
...
DimPlot(seu, group.by = "NK_cluster") + NoAxes()
...
DimPlot(seu, reduction = "umap.scvi", group.by = "NK_cluster") + NoAxes()

saveRDS(seu, file = "RDSfiles/seu_NK_downloaded.RDS")
