...
# load data of T cells----
cts <- ReadMtx(mtx = "downloaded/T_cells/matrix.mtx.gz", features = "downloaded/T_cells/features.tsv.gz", cells = "downloaded/T_cells/barcodes.tsv.gz")
meta <- read.delim("downloaded/T_cells/meta_data.tsv", stringsAsFactors = F, header = T)
...
DimPlot(seu, group.by = "T_cluster") + NoAxes()
...
DimPlot(seu, reduction = "umap.scvi", group.by = "T_cluster") + NoAxes()

saveRDS(seu, file = "RDSfiles/seu_T_downloaded.RDS")
